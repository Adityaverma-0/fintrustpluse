...
router.post("/wallet/deposit", requireAuth, async (req, res) => {
  ...
  const [txRecord] = await db.insert(walletTransactionsTable).values({
    walletId: wallet.id,
    type: "credit",
  ...
  try {
    await generateInvoiceForEvent("wallet_deposit", amount, req.userId!, undefined, {
      transactionId: txRecord.id,
      paymentMethod: method ?? "Mock Deposit",
    });
  } catch (err) {
    console.error("Wallet deposit invoice generation failed:", err);
  }
...
router.post("/wallet/withdraw-mock", requireAuth, async (req, res) => {
  const parsed = z.object({
    amount: z.number().positive(),
    method: z.string().optional(),
  }).safeParse(req.body);
  ...
  try {
    await generateInvoiceForEvent("withdrawal", amount, req.userId!, undefined, {
      transactionId: txRecord.id,
      paymentMethod: method ?? "Mock Withdrawal",
    });
  } catch (err) {
    console.error("Wallet withdrawal invoice generation failed:", err);
  }
